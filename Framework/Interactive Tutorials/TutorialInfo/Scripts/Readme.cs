...
public class Readme : ScriptableObject
{
    ...
    internal const string k_DefaultLayout =
    ...
    public void LoadTutorialProjectLayout(bool force)
    {
        if (force || !loadedLayout)
        {
            var layoutPath = projectLayout != null
                ? AssetDatabase.GetAssetPath(projectLayout)
                : k_DefaultLayout;
            loadedLayout  = TutorialManager.LoadWindowLayout(layoutPath);
        }
    }
}
